...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for layer, target, weight in zip(style_layers, style_targets, style_weights):
        ...
        style_loss += weight * (curr_gram - target).square().sum()
    return style_loss
    ############################################################################
    #                               END OF YOUR CODE                           #
    ############################################################################
...
def guided_gram_matrix(features, masks, normalize=True):
  ...
  masked_features = masks.unsqueeze(2) * features
...
def guided_style_loss(feats, style_layers, style_targets, style_weights, content_masks):
    ...
    # Replace "pass" statement with your code
    style_loss = 0
    for layer, target, weight, mask in zip(style_layers, style_targets, style_weights, content_masks):
        curr_gram = guided_gram_matrix(feats[layer], content_masks[layer])
        style_loss += weight * (curr_gram - target).square().sum()
    return style_loss
