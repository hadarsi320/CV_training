...
def compute_saliency_maps(X, y, model):
    ...
    pred = model(X)
    loss = F.cross_entropy(pred, y) * X.shape[0]
    loss.backward()

    saliency = X.grad.abs().amax(1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return saliency
...
def make_adversarial_attack(X, target_y, model, max_iter=100, verbose=True):
    ...
    y = torch.tensor([target_y], device=X.device)
    for i in range(max_iter):
        pred = model(X_adv)
        max_score = pred.amax().item()
        target_score = pred[0, target_y].item()
        if verbose:
            print('Iteration {}: target score {:.3f}, max score {:.3f}'
                .format(i+1, target_score, max_score))
        if max_score == target_score:
            break
        
        loss = F.cross_entropy(pred, y)
        loss.backward()
        dX = learning_rate * X_adv.grad / X_adv.grad.norm()
        X_adv.grad.zero_()
        X_adv = X_adv - dX
        X_adv.retain_grad()
    ##############################################################################
...
def class_visualization_step(img, target_y, model, **kwargs):
    ...
    obj = pred[0, target_y]
    if l2_reg > 0:
        obj -= l2_reg * img.square().sum()
    obj.backward()
    img.data.add_(learning_rate * img.grad)
    img.grad.zero_()
    ########################################################################
    #                             END OF YOUR CODE                         #
    ########################################################################
