...
class VAE(nn.Module):
    def __init__(self, input_size, latent_size=15):
        ...
        self.hidden_dim = 400 # what the fuck
        encoder = [nn.Flatten(1)]
        last_dim = input_size
        for _ in range(3):
            encoder.append(nn.Linear(last_dim, self.hidden_dim))
        ...
        # Replace "pass" statement with your code
        decoder = []
        last_dim = latent_size
        for _ in range(3):
            decoder.append(nn.Linear(last_dim, self.hidden_dim))
            decoder.append(nn.ReLU())
            last_dim = self.hidden_dim
        H = int(input_size ** 0.5)
        decoder.extend([nn.Linear(last_dim, input_size), 
                        nn.Sigmoid(), 
                        nn.Unflatten(1, (1, H, H))])
...
class CVAE(nn.Module):
    def __init__(self, input_size, num_classes=10, latent_size=15):
        ...
        self.hidden_dim = 400 # what the fuck
        encoder = []
        last_dim = input_size + num_classes
        for _ in range(3):
            encoder.append(nn.Linear(last_dim, self.hidden_dim))
            encoder.append(nn.ReLU())
            last_dim = self.hidden_dim
        ...
        # Replace "pass" statement with your code
        decoder = []
        last_dim = latent_size + num_classes
        for _ in range(3):
            decoder.append(nn.Linear(last_dim, self.hidden_dim))
            decoder.append(nn.ReLU())
            last_dim = self.hidden_dim
    ...
    def forward(self, x, c):
        ...
        ###########################################################################
        # Replace "pass" statement with your code
        enc_inp = torch.cat([torch.flatten(x, 1), c], -1)
        hidden = self.encoder(enc_inp)
        mu = self.mu_layer(hidden)
        logvar = self.logvar_layer(hidden)
        z = reparametrize(mu, logvar)
        dec_inp = torch.cat([z, c], -1)
...
def reparametrize(mu, logvar):
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    z = mu + torch.randn_like(mu) * logvar.exp().sqrt()
    ###############################################################################
    #                              END OF YOUR CODE                               #
    ###############################################################################
    return z
...
def loss_function(x_hat, x, mu, logvar):
    ...
    loss = (reconstruction_loss + kl_divergence) / N
    ###############################################################################
    #                            END OF YOUR CODE                                 #
    ###############################################################################
    return loss
