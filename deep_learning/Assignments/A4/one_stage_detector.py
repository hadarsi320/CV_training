...
class FCOSPredictionNetwork(nn.Module):
    ...
    def __init__(
        ...
        def create_conv(last_channels, channels):
            conv = nn.Conv2d(last_channels, channels, 3, padding=1)
        ...
        for channels in stem_channels:
            for stem in [stem_cls, stem_box]:
                stem.append(create_conv(last_channels, channels))
                stem.append(nn.ReLU())
        ...
        # Replace "pass" statement with your code
        self.pred_cls = nn.Conv2d(last_channels, num_classes, 3, padding=1)
        self.pred_box = nn.Conv2d(last_channels, 4, 3, padding=1)
        self.pred_ctr = nn.Conv2d(last_channels, 1, 3, padding=1)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def forward(self, feats_per_fpn_level: TensorDict) -> List[TensorDict]:
        ...
        # Replace "pass" statement with your code
        def permute_flatten(tensor):
            tensor = torch.permute(tensor, [0, 2, 3, 1])
            tensor = tensor.reshape(tensor.shape[0], -1, tensor.shape[-1])
            return tensor

        for key, feats in feats_per_fpn_level.items():
            cls_feats = self.stem_cls(feats)
            class_logits[key] = permute_flatten(self.pred_cls(cls_feats))

            box_feats = self.stem_box(feats)
            boxreg_deltas[key] = permute_flatten(self.pred_box(box_feats))
            centerness_logits[key] = permute_flatten(self.pred_ctr(box_feats))
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
...
def fcos_apply_deltas_to_locations(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    deltas = torch.clamp(deltas, min=0) * stride
...
def fcos_make_centerness_targets(deltas: torch.Tensor):
    ...
    lr = deltas[..., [0, 2]]
    tb = deltas[..., [1, 3]]
    centerness = lr.amin(-1) * tb.amin(-1) / (lr.amax(-1) * tb.amax(-1))
    centerness = centerness.sqrt()

    background = (deltas == -1).all(-1)
...
class FCOS(nn.Module):
    ...
    def forward(
        ...
        ######################################################################
        # Feel free to delete this line: (but keep variable names same)
        shapes = {key: tensor.shape for key, tensor in backbone_feats.items()}
        locations_per_fpn_level = get_fpn_location_coords(
          shapes, self.backbone.fpn_strides, images.dtype, images.device)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        matched_gt_boxes = []
        # Replace "pass" statement with your code
        for img_gt_boxes in gt_boxes:
          matched_gt_boxes.append(fcos_match_locations_to_gt(
          locations_per_fpn_level, self.backbone.fpn_strides, img_gt_boxes))

        # Calculate GT deltas for these matched boxes. Similar structure
        # as `matched_gt_boxes` above. Fill this list:
        matched_gt_deltas = []
        # Replace "pass" statement with your code
        for img_gt_boxes in matched_gt_boxes:
          matched_gt_deltas.append({})
          for key, stride in self.backbone.fpn_strides.items():
            matched_gt_deltas[-1][key] = fcos_get_deltas_from_locations(
              locations_per_fpn_level[key], img_gt_boxes[key], stride)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
        ...
        # Replace "pass" statement with your code
        background = matched_gt_boxes[..., 0] == -1
        matched_cls = matched_gt_boxes[..., -1].to(int)
        one_hot_cls = F.one_hot(matched_cls.clip(min=0), num_classes=self.num_classes).to(float)
        one_hot_cls[background] = 0

        loss_cls = sigmoid_focal_loss(inputs=pred_cls_logits, targets=one_hot_cls)
        ...
        loss_ctr = F.binary_cross_entropy_with_logits(pred_ctr_logits.squeeze(), centerness, reduction='none')

        loss_box[background] = 0
        loss_ctr[background] = 0

        # assertions
        # for b in range(matched_gt_boxes.shape[0]):
        #   for i in range(matched_gt_boxes.shape[1]):
        #     is_background = (matched_gt_boxes[b, i] == -1).all()
        #     if is_background:
        #       assert ((one_hot_cls[b, i] == 0).all() and (loss_box[b, i] == 0).all() and loss_ctr[b, i] == 0)
        #     else:
        #       cls = int(matched_gt_boxes[b, i, -1].item())
        #       assert one_hot_cls[b, i, cls] == 1 and (loss_box[b, i] != 0).all() and loss_ctr[b, i] != 0
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def inference(
        ...
        for level_name in locations_per_fpn_level.keys():
            ...
            # Step 2:
            # Replace "pass" statement with your code
            valid_predictions = level_pred_scores > test_score_thresh
            level_pred_scores = level_pred_scores[valid_predictions]
            level_pred_classes = level_pred_classes[valid_predictions]

            # Step 3:
            # Replace "pass" statement with your code
            stride = self.backbone.fpn_strides[level_name]
            level_pred_boxes = fcos_apply_deltas_to_locations(
              level_deltas, level_locations, stride)[valid_predictions]
