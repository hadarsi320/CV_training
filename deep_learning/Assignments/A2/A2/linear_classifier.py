...
def svm_loss_vectorized(
    ...
    C = W.shape[1]
    scores = X @ W  # (N, C)
    correct_class_scores = scores[torch.arange(N), y].view(N, 1)  # (N, 1)
    margins = scores - correct_class_scores + 1  # (N, C)
    margins[torch.arange(N), y] = 0
    margins = torch.maximum(margins, torch.zeros_like(margins))
    loss = margins.sum() / N 
    loss += reg * torch.sum(W * W)
    #############################################################################
    #                             END OF YOUR CODE                              #
    ...
    dW += X.T @ (margins > 0).to(X.dtype)
    ii = torch.zeros_like(margins)
    ii[torch.arange(N), y] = 1
    dW -= (X * torch.sum(margins > 0, dim=1, keepdim=True)).T @ ii
    dW /= N
    dW += 2 * reg * W
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    index_sample = torch.randint(num_train, (batch_size,))
    X_batch = X[index_sample]
    y_batch = y[index_sample]
    #########################################################################
    #                       END OF YOUR CODE                                #
    #########################################################################
...
def svm_get_search_params():
    ...
    regularization_strengths = [5e-2, 1e-1, 5e-1]
    ###########################################################################
    #                           END OF YOUR CODE                              #
    ###########################################################################
...
def test_one_param_set(
    ...
    pred = cls.predict(data_dict['X_train'])
    train_acc = (pred == data_dict['y_train']).double().mean().item()
    pred = cls.predict(data_dict['X_val'])
    val_acc = (pred == data_dict['y_val']).double().mean().item()
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(N):
        ...
        odds = logits.exp()  # (C,)
        probas = odds / odds.sum()  # (C,)
        log_probas = probas.log()  # (C,)
        loss += - log_probas[y[i]]

        for j in range(C):
          dW[:, j] += X[i] * probas[j]
        # dW += X[i].unsqueeze(1) * probas.unsqueeze(0)
        dW[:, y[i]] -= X[i]
    loss /= N
    loss += reg * torch.sum(W * W)
    dW /= N
...
def softmax_loss_vectorized(
    ...
    N = X.shape[0]
    logits = X @ W                                        # (N, C)
    logits -= logits.max(axis=1, keepdim=True)[0]
    odds = logits.exp()                                   # (N, C)
    probas = odds / odds.sum(1, keepdim=True)             # (N, C)
    log_probas = probas.log()                             # (N, C)
    loss = - log_probas[torch.arange(N), y].mean()
    loss += reg * torch.sum(W * W)

    dW += X.T @ probas
    ii = torch.zeros_like(logits)                         # (N, C)
    ii[torch.arange(N), y] = 1
    dW -= X.T @ ii
    dW /= N
    dW += 2 * reg * W
...
def softmax_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    learning_rates = [1e-2, 5e-2, 1e-1, 5e-1, 1e0]
