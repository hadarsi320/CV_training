...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden = X @ W1 + b1  # linear
    hidden[hidden < 0] = 0  # relu
    scores = hidden @ W2 + b2  # linear
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def nn_forward_backward(
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    grads = {k: torch.zeros_like(params[k]) for k in params}

    ii = torch.zeros_like(probas)
...
def nn_train(
    ...
    for it in range(num_iters):
        ...
        # Replace "pass" statement with your code
        for k in params:
            params[k] -= grads[k] * learning_rate
        #########################################################################
        #                             END OF YOUR CODE                          #
        #########################################################################
...
def find_best_net(
    ...
    i = 1
    total = len(learning_rates) * len(hidden_sizes) * \
      len(regularization_strengths) * len(learning_rate_decays)
    for lr in learning_rates:
        for hs in hidden_sizes:
            for lr_decay in learning_rate_decays:
              for reg in regularization_strengths:
                    print('Training model {}/{} with hs={} lr={:.2e} lr decay={:.2f} reg={:.2e}'
                      .format(i, total, hs, lr, lr_decay, reg))
                    ...
                    stats = net.train(data_dict['X_train'], data_dict['y_train'], 
                      data_dict['X_val'], data_dict['y_val'], num_iters=3000,
                      batch_size=1000, learning_rate=lr, 
                      learning_rate_decay=lr_decay, reg=reg, verbose=False)
