...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out = torch.empty((N, F, oH, oW), device=x.device, dtype=x.dtype)
        x_pad = torch.nn.functional.pad(x, tuple([pad] * 4))
        for i in range(oH):
            for j in range(oW):
                H_slice = slice(i * stride, i * stride + wH)
                W_slice = slice(j * stride, j * stride + wW)
                window = x_pad[..., H_slice, W_slice]
                res = (window.unsqueeze(1) * w).sum((2, 3, 4))
                out[..., i, j] = res
        out += b.view(1, -1, 1, 1)
        #####################################################################
        #                          END OF YOUR CODE                         #
    ...
    def backward(dout, cache):
        ...
        N, C, H, W = x.shape
        F, _, wH, wW = w.shape
        x = torch.nn.functional.pad(x, tuple([pad] * 4))
        ...
        for i in range(dout.shape[-2]):
            for j in range(dout.shape[-1]):
                H_slice = slice(i * stride, i * stride + wH)
                W_slice = slice(j * stride, j * stride + wW)
                dwindow = dx[..., H_slice, W_slice]
                window = x[..., H_slice, W_slice]
                dp = dout[..., i, j].view(N, F, 1, 1, 1)

                dwindow += (dp * w).sum(1)                
                dw += (dp * window.unsqueeze(1)).sum(0)
...
class MaxPool(object):
    ...
    def forward(x, pool_param):
        ...
        for i in range(oH):
            for j in range(oW):
                H_slice = slice(i * stride, i * stride + pH)
                W_slice = slice(j * stride, j * stride + pW)
                window = x[..., H_slice, W_slice]
                out[..., i, j] = window.amax((-2, -1))
        ####################################################################
        #                         END OF YOUR CODE                         #
        ####################################################################
        cache = (x, pool_param)
    ...
    def backward(dout, cache):
        ...
        for i in range(dout.shape[-2]):
            for j in range(dout.shape[-1]):
                ...
                window = x[..., H_slice, W_slice]
                dwindow = dx[..., H_slice, W_slice]
                wmax = window.amax((-2, -1)).unsqueeze(-1).unsqueeze(-1)
                dwindow[window == wmax] = dout[..., i, j].view(-1)
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        ######################################################################
        # Replace "pass" statement with your code
        def create_param(shape, bias=False):
            if bias:
                return torch.zeros(shape, dtype=self.dtype, device=device)
            return torch.randn(shape, dtype=self.dtype, device=device) * weight_scale

        filter_shape = (num_filters, input_dims[0], filter_size, filter_size)
        conv_output_shape = num_filters * (input_dims[1] // 2) * (input_dims[2] // 2)
        self.params['W1'] = create_param(filter_shape)
        self.params['b1'] = create_param((num_filters,), True)
        self.params['W2'] = create_param((conv_output_shape, hidden_dim))
        self.params['b2'] = create_param((hidden_dim,), True)
        self.params['W3'] = create_param((hidden_dim, num_classes))
        self.params['b3'] = create_param((num_classes,), True)
    ...
    def loss(self, X, y=None):
        ...
        X, lin1_cache = Linear_ReLU.forward(X, W2, b2)
        X, lin2_cache = Linear.forward(X, W3, b3)
        scores = X
        ######################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        dx, dw, db = Linear.backward(dL, lin2_cache)
        grads['W3'] = dw
        grads['b3'] = db
        dx, dw, db = Linear_ReLU.backward(dx, lin1_cache)
        grads['W2'] = dw
        grads['b2'] = db
        _, dw, db = Conv_ReLU_Pool.backward(dx, conv_cache)
        grads['W1'] = dw
        grads['b1'] = db
        ...
        for key, param in self.params.items():
            if 'W' in key:
              loss += self.reg * param.square().sum()
              grads[key] += 2 * self.reg * param
        ###################################################################
        #                             END OF YOUR CODE                    #
        ###################################################################
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        args = dict(dtype=self.dtype, device=device)
        ...
        def create_param(shape):
            if weight_scale == 'kaiming':
                ...
                scale = weight_scale
            return torch.randn(shape, **args) * scale

        x_shape = list(input_dims)
        for i, filters in enumerate(num_filters):
            self.params[f'W{i}'] = create_param((filters, x_shape[0], 3, 3))
            self.params[f'b{i}'] = torch.zeros(filters, **args)
            if self.batchnorm:
                self.params[f'gamma{i}'] = torch.ones(filters, **args)
                self.params[f'beta{i}'] = torch.zeros(filters, **args)
            x_shape[0] = filters
            if i in self.max_pools:
                x_shape[1:] = [x_shape[1] // 2, x_shape[2] // 2]

        self.params[f'W{i+1}'] = create_param((prod(x_shape), num_classes))
        self.params[f'b{i+1}'] = torch.zeros(num_classes, **args)
        ################################################################
        #                      END OF YOUR CODE                        #
        ################################################################
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers-1, -1, -1):
            ...
            dx, dw, db, *extra = layer.backward(dx, caches.pop())
            grads[f'W{i}'] = dw
            grads[f'b{i}'] = db
            if self.batchnorm and i < self.num_layers - 1:
                grads[f'gamma{i}'] = extra[0]
                grads[f'beta{i}'] = extra[1]
...
def create_convolutional_solver_instance(data_dict, dtype, device, 
      lr=5e-3, reg=1e-3, n_layers=3, first_filter=16):
    model = None
    solver = None
    #########################################################
    ...
    for i in range(n_layers-1):
        filters.append(2 * filters[-1])
    max_pools = list(range(n_layers))

    model = DeepConvNet(num_filters=filters, max_pools=max_pools, reg=reg,
      weight_scale='kaiming', dtype=dtype, device=device)
    # sgd: lr=1e-2
    # adam: lr=1e-2
    solver = Solver(model, data_dict, device=device, update_rule=adam, 
        lr_decay=0.95, optim_config={'learning_rate': lr},
        batch_size=128, print_every=10000, print_acc_every=5, num_epochs=30)
    #########################################################
    #                  END OF YOUR CODE                     #
    #########################################################
    return solver
...
class BatchNorm(object):
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        x0, denom, x_norm, gamma = cache
        N = x0.shape[0]
        dx_norm = dout * gamma
        dvar = - (dx_norm * x0).sum(0) / (2 * denom ** 3)
        dmean = - (dx_norm / denom).sum(0) - dvar * 2 * x0.mean(0)
        dx = dx_norm / denom + dvar * 2 * x0 / N + dmean / N
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        ################################################################
        # Replace "pass" statement with your code
        x_t = torch.transpose(x, 1, 3)
        x_inp = x_t.reshape(-1, x_t.shape[-1])
        out, cache = BatchNorm.forward(x_inp, gamma, beta, bn_param)
    ...
    def backward(dout, cache):
        ...
        #################################################################
        # Replace "pass" statement with your code
        dout_t = torch.transpose(dout, 1, 3)
        dout_inp = dout_t.reshape(-1, dout.shape[1])
        dx, dgamma, dbeta = BatchNorm.backward(dout_inp, cache)
        dx = dx.reshape(*dout_t.shape)
        dx = torch.transpose(dx, 1, 3)
