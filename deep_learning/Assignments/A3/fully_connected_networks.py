...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        def create_param(size, bias=False):
            if bias:
              return torch.zeros(size, dtype=dtype, device=device)
            return torch.randn(size, dtype=dtype, device=device) * weight_scale

        self.params['W1'] = create_param((input_dim, hidden_dim))
        self.params['b1'] = create_param((hidden_dim,), True)
        self.params['W2'] = create_param((hidden_dim, num_classes))
        self.params['b2'] = create_param((num_classes,), True)
    ...
    def loss(self, X, y=None):
        ...
        #############################################################
        # Replace "pass" statement with your code
        X, lin1_cache = Linear.forward(X, self.params['W1'], self.params['b1'])
        X, relu_cache = ReLU.forward(X)
        X, lin2_cache = Linear.forward(X, self.params['W2'], self.params['b2'])
        scores = X
        ##############################################################
        #                     END OF YOUR CODE                       #
        ##############################################################
        ...
        _, dw, db = Linear.backward(dx, lin1_cache)
        grads['W1'] = dw
        grads['b1'] = db
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        for i, dim in enumerate(dims):
            self.params[f'W{i}'] = create_param((last_dim, dim))
            self.params[f'b{i}'] = create_param((dim,), True)
            last_dim = dim
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers):
            X, cache = Linear.forward(X, self.params[f'W{i}'], self.params[f'b{i}'])
            caches.append(cache)
            if i + 1 < self.num_layers:
              X, cache = ReLU.forward(X)
              caches.append(cache)
              if self.use_dropout:
        ...
        for i in range(self.num_layers):
            if i > 0:
                if self.use_dropout:
                    dx = Dropout.backward(dx, caches.pop(-1))
                dx = ReLU.backward(dx, caches.pop(-1))
            pi = self.num_layers - i - 1
            dx, dw, db = Linear.backward(dx, caches.pop(-1))
            grads[f'W{pi}'] = dw
            grads[f'b{pi}'] = db
...
def create_solver_instance(data_dict, dtype, device):
    ...
    solver = None
    # Replace "pass" statement with your code
    solver = Solver(model, data_dict, device=device, lr_decay=0.98, 
      optim_config={'learning_rate': 2e-1}, print_every=100)
    ##############################################################
    #                    END OF YOUR CODE                        #
    ##############################################################
    return solver
