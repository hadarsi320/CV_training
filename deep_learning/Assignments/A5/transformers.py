...
def generate_token_dict(vocab):
    ...
    token_dict = {item: i for i, item in enumerate(vocab)}
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return token_dict
...
def prepocess_input_sequence(
    ...
    for string in input_str.split():
        if string in spc_tokens:
            out.append(token_dict[string])
        else:
            digit_tokens = [token_dict[digit] for digit in string]
            out.extend(digit_tokens)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_single(
    ...
    for i in range(K):
        similarities = []
        for j in range(K):
            similarities.append(query[i] @ key[j] / M ** 0.5)
        similarities = F.softmax(torch.tensor(similarities), 0)
        out[i] = similarities @ value
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_two_loop_batch(
    ...
    for i in range(K):
        similarities = []
        for j in range(K):
            sim = torch.bmm(query[:, i].unsqueeze(1), key[:, j].unsqueeze(2)).squeeze((-2, -1))
            similarities.append(sim / M ** 0.5)
        similarities = F.softmax(torch.stack(similarities, 1), 1)
        out[:, i] = torch.bmm(similarities.unsqueeze(1), value).squeeze(1)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    ###############################################################################
    # Replace "pass" statement with your code
    weights = torch.bmm(query, key.transpose(-2, -1)) / M ** 0.5
    if mask is not None:
        ##########################################################################
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        weights = weights.masked_fill_(mask, -1e9)

    # Replace "pass" statement with your code
    weights_softmax = F.softmax(weights, 2)
...
class SelfAttention(nn.Module):
    def __init__(self, dim_in: int, dim_q: int, dim_v: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        def create_linear(dim_out):
            linear = nn.Linear(dim_in, dim_out)
            c = math.sqrt(6 / (dim_in + dim_out))
            torch.nn.init.uniform_(linear.weight, -c, c)
            return linear

        self.q = create_linear(dim_q)
        self.k = create_linear(dim_q)
        self.v = create_linear(dim_v)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        NOTE: Here, when we say dimension, we mean the dimesnion of the embeddings.
              ...
              don't change usually, we mostly transform the dimension(M) dimension.
        """
        ...
        self.projector = nn.Linear(dim_out * num_heads, dim_in)
        c = math.sqrt(6 / (self.projector.in_features + self.projector.out_features))
        torch.nn.init.uniform_(self.projector.weight, -c, c)
    ...
    def forward(
        ...
        ##########################################################################
        # Replace "pass" statement with your code4
        attn_out = torch.cat([
            head(query, key, value, mask) for head in self.attention_heads
        ], -1)
        y = self.projector(attn_out)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        ##########################################################################
        # Replace "pass" statement with your code
        def create_linear(dim_in, dim_out):
            linear = nn.Linear(dim_in, dim_out)
            c = math.sqrt(6 / (dim_in + dim_out))
            torch.nn.init.uniform_(linear.weight, -c, c)
            return linear

        self.lin1 = create_linear(inp_dim, hidden_dim_feedforward)
        self.lin2 = create_linear(hidden_dim_feedforward, inp_dim)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class EncoderBlock(nn.Module):
    ...
    def forward(self, x):
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        attn_out = self.dropout(self.norm1(attn_out + x))
        ff_out = self.feedforward(attn_out)
        y = self.dropout(self.norm2(ff_out + attn_out))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def get_subsequent_mask(seq):
    ...
    indices = torch.tril_indices(K, K, device=seq.device)
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        As shown in the Figure 1 of the paper attention is all you need
        https://arxiv.org/pdf/1706.03762.pdf, the decoder consists of 5 components:   
        ...
        attn_args = (num_heads, emb_dim, emb_dim // num_heads)
        self.attention_self = MultiHeadAttention(*attn_args)
        self.attention_cross = MultiHeadAttention(*attn_args)
    ...
    def forward(
        ...
        self_attn_out = self.dropout(self.norm1(dec_inp + self_attn_out))
        cross_attn_out = self.attention_cross(self_attn_out, enc_inp, enc_inp)
        cross_attn_out = self.dropout(self.norm2(self_attn_out + cross_attn_out))
        ff_out = self.feed_forward(cross_attn_out)
        y = self.dropout(self.norm3(cross_attn_out + ff_out))

        # for head in self.attention_self.attention_heads:
        #     print(head.weights_softmax)
        # for head in self.attention_cross.attention_heads:
        #     print(head.weights_softmax)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    y = torch.linspace(0, 1, steps=K + 1)[:-1].view(1, -1, 1).repeat(1, 1, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # Replace "pass" statement with your code
    y = torch.empty(1, K, M)
    ii = torch.arange(math.ceil(M / 2))
    a = torch.floor(2 * ii / M)
    inp = torch.outer(torch.arange(K), 1 / (10000 ** a))
    sin = torch.sin(inp)
    cos = torch.cos(inp)
    if M % 2 == 1:
        cos = cos[:, :-1]
