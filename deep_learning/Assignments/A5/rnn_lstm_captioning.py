...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = F.tanh(x @ Wx + prev_h @ Wh + b)
...
def rnn_backward(dh, cache):
    ...
    for t in reversed(range(dh.shape[1])):
        dh_t += dh[:, t]
        dx_, dh_t, dWx_, dWh_, db_ = rnn_step_backward(dh_t, cache[t])
        dx[t] += dx_
        dWx += dWx_
        dWh += dWh_
        db += db_
...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = F.cross_entropy(x.reshape(-1, x.shape[-1]), y.reshape(-1),
...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        assert input_dim == self.image_encoder.out_channels
        if self.cell_type in ['rnn', 'lstm']:
            cls = RNN if self.cell_type == 'rnn' else LSTM
            self.feature_proj = nn.Linear(input_dim, hidden_dim)
        else:
            cls = AttentionLSTM
            self.feature_proj = nn.Conv2d(input_dim, hidden_dim, 1)
        self.rnn = cls(wordvec_dim, hidden_dim)
        ######################################################################
        #                            END OF YOUR CODE                        #
        ######################################################################
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type != 'attn':
            image_feats = image_feats.mean([-2, -1])
        h0 = self.feature_proj(image_feats)

        # 2 
        embeddings = self.embedding(captions_in)
        
        # 3
        hidden_states = self.rnn(embeddings, h0)

        # 4
        vocab_scores = self.output_proj(hidden_states)

        # 5
        loss = temporal_softmax_loss(vocab_scores, captions_out, 
            self.ignore_index)
        ######################################################################
        #                           END OF YOUR CODE                         #
        ######################################################################
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type != 'attn':
            image_feats = image_feats.mean([-2, -1])
        A = self.feature_proj(image_feats)
        if self.cell_type == 'attn':
            h_t = A.mean([-2, -1])
        else:
            h_t = A
        ...
        ended = torch.tensor([False] * N, device=device)
        word_t = torch.tensor([self._start] * N, device=device).view(-1)
        args = []
        if self.cell_type == 'lstm':
            args.append(torch.zeros_like(h_t))
        elif self.cell_type == 'attn':
            args.append(h_t)

        for i in range(max_length):
            # 1
            embeddings = self.embedding(word_t)

            # 2
            if self.cell_type == 'attn':
                attn, attn_weights = dot_product_attention(h_t, A)
                args.append(attn)
                attn_weights_all[:, i] = attn_weights
            h_t = self.rnn.step_forward(embeddings, h_t, *args)
            if isinstance(h_t, tuple):
                h_t, *args = h_t

            # 3
            vocab_scores = self.output_proj(h_t)
                
            # 4
            word_t = vocab_scores.max(-1).indices
            captions[~ended, i] = word_t[~ended]
            ended[word_t == self._end] = True
            if ended.all():
                break
        ######################################################################
        #                           END OF YOUR CODE                         #
...
class LSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, h0: torch.Tensor) -> torch.Tensor:
        ...
        for t in range(x.shape[1]):
            h_t, c_t = self.step_forward(x[:, t], h_t, c_t)
            hn.append(h_t)
        hn = torch.stack(hn, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
...
def dot_product_attention(prev_h, A):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    prev_h = prev_h.view(N, 1, H)
    A = A.view(N, H, -1)
    attn_weights = torch.bmm(prev_h, A) / math.sqrt(H)
    attn_weights = F.softmax(attn_weights, -1)
    attn = torch.bmm(A, attn_weights.transpose(1, 2)).squeeze(-1)
...
class AttentionLSTM(nn.Module):
    ...
    def forward(self, x: torch.Tensor, A: torch.Tensor):
        ...
        for t in range(x.shape[1]):
            ...
            h_t, c_t = self.step_forward(x[:, t], h_t, c_t, attn)
            hn.append(h_t)
        hn = torch.stack(hn, 1)
        ######################################################################
        #                           END OF YOUR CODE                         #
